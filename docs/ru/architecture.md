...
[← К README](../../README.ru.md) · [API](./api.md) · [Механика аукционов](./auction-mechanics.md) · [Конкурентность](./concurrency.md) · [Тестирование](./testing.md) · [Деплой](./deployment.md)

---

## Показатели производительности

### Один процесс (1 воркер)

| Протокол | Пик | Устойчивая | Латентность |
|----------|-----|------------|-------------|
| **WebSocket** | **200,018 emit/s** | 175,970/s | 0мс |
| **HTTP** | **3,362 req/s** | 3,100/s | 1.5мс mean, 5мс p99 |

### Кластерный режим (12 ядер)

| Протокол | Пик | Улучшение |
|----------|-----|-----------|
| **HTTP** | **13,812 req/s** | ~4.1x |
| **WebSocket** | ~2.4M emit/s | теоретически (требуется sticky sessions) |

**Включить кластер**: `CLUSTER_WORKERS=auto` в `.env`

Полные бенчмарки: [BENCHMARK_REPORT.md](../../backend/test/artillery/BENCHMARK_REPORT.md) · [Отчёты](https://telegram-gift-auction.funfiesta.games/api/reports/)
...
│  │  └─ getTop() [ZRANGE]                                        │  │
│  │                                                               │  │
│  │  BidCacheService (Высокопроизводительный путь)               │  │
│  │  ├─ placeBidFast()        [Единый Lua скрипт]                │  │
│  │  ├─ warmupAuctionCache()                                     │  │
│  │  └─ getAuctionMeta()                                         │  │
│  │                                                               │  │
...
**Максимальная пропускная способность: 200,000 emit/sec пик, 176,000/sec устойчивая с 0мс латентностью**
...
### Почему единый Lua скрипт для высокопроизводительных ставок?
...
- Обеспечивает 3,362 rps (1 ядро), 13,812 rps (12 ядер) с латентностью 1.4мс
...
- В сочетании с Lua скриптом: **200,000 emit/sec пик** с суб-миллисекундной латентностью
