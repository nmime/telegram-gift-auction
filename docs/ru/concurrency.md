...
## Высокопроизводительный путь ставки (Redis Lua скрипт)

Эндпоинт `/bid` использует высокопроизводительный Redis Lua скрипт для максимальной пропускной способности:
...
| Метрика | Значение |
|---------|----------|
| Латентность | 1.4мс mean, 4мс p99 |
| Пиковая пропускная способность | 3,362 req/s (1 ядро), 13,812 req/s (12 ядер) |
| Консистентность | Eventual (5с синхр. в MongoDB) |
| Защита | Атомарный Lua + Dirty Tracking |

---

## Детали потока ставки
