...
[← К README](../../README.ru.md) · [Архитектура](./architecture.md) · [API](./api.md) · [Конкурентность](./concurrency.md) · [Деплой](./deployment.md)
...
## Artillery нагрузочные тесты (v2.0.27)

Продакшен-grade нагрузочное тестирование с Artillery для реалистичной симуляции трафика.
...
pnpm run load-test:stress    # Экстремальный стресс-тест
...
| Протокол | Пик | Устойчивая | Латентность | Оценка |
|----------|-----|------------|-------------|--------|
| **WebSocket** | **200,018 emit/s** | 175,970/s | 0мс | **A+** |
| **HTTP** | **3,362 req/s** | 3,100/s | 1.5мс mean, 5мс p99 | **A+** |
...
| **WebSocket** | ~2.4M emit/s | теоретически |

Включить кластер: `CLUSTER_WORKERS=auto` в `.env`

### Результаты максимальной пропускной способности

```
╔══════════════════════════════════════════════════════════════╗
║  WEBSOCKET (один процесс)                                    ║
║    🚀 ПИК:        200,018 emit/sec                          ║
║    ⚡ УСТОЙЧИВАЯ: 175,970 emit/sec                           ║
║    📊 ВСЕГО:      11,305,542 emits за 67 секунд             ║
║    ⏱️  ЛАТЕНТНОСТЬ: 0мс (суб-миллисекундная)                 ║
║                                                              ║
║  HTTP (12-ядерный кластер)                                   ║
║    🚀 ПИК:        13,812 req/sec                            ║
║    ⚡ УСТОЙЧИВАЯ: 12,000-13,000 req/sec                      ║
║    📈 УЛУЧШЕНИЕ:  ~4.1x по сравнению с одним процессом       ║
╚══════════════════════════════════════════════════════════════╝
```
...
Полные детали бенчмарков: [`backend/test/artillery/BENCHMARK_REPORT.md`](../../backend/test/artillery/BENCHMARK_REPORT.md)
