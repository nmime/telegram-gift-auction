...
### Результаты производительности (один процесс)

| Протокол | Пик | Устойчивая | Латентность | Оценка |
|----------|-----|------------|-------------|--------|
| **WebSocket** | **62,951 emit/s** | 43,056/s | 0мс | **A+** |
| **HTTP (raw)** | 600 req/s | 140 req/s | 18мс | A |
| **HTTP (с лимитами)** | 138 req/s | 138 req/s | 18мс | A |

### Результаты экстремального WebSocket теста

```
╔══════════════════════════════════════════════════════════════╗
║  🚀 ПИК:        62,951 emit/sec                             ║
║  ⚡ УСТОЙЧИВАЯ: 43,056 emit/sec                              ║
║  📊 ВСЕГО:      2,581,008 emits за 90 секунд                ║
...
```
test/artillery/
├── load-test.yml           # HTTP нагрузочный тест
├── stress-test.yml         # HTTP стресс-тест
