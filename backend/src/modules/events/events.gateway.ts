...
export class EventsGateway {
  ...
  private handleConnection(client: Socket) {
    ...
    client.on('join-auction', (auctionId: string) => this.handleJoinAuction(client, auctionId));
    client.on('leave-auction', (auctionId: string) => this.handleLeaveAuction(client, auctionId));
  }
  ...
  private handleJoinAuction(client: Socket, auctionId: string) {
    ...
    this.logger.debug('Client joined auction', { clientId: client.id, auctionId });
  ...
  private handleLeaveAuction(client: Socket, auctionId: string) {
    ...
    this.logger.debug('Client left auction', { clientId: client.id, auctionId });
  ...
  emitNewBid(auctionId: string, bidInfo: { amount: number; timestamp: Date; isIncrease: boolean }) {
    if (!this.server) return;
    const room = `auction:${auctionId}`;
    const roomSize = this.server.sockets.adapter.rooms.get(room)?.size || 0;
    this.logger.debug('Emitting new-bid', { auctionId, amount: bidInfo.amount, roomSize });
  ...
  emitRoundComplete(auction: AuctionDocument, roundNumber: number, winners: BidDocument[]) {
    if (!this.server) return;
  ...
  emitAuctionComplete(auction: AuctionDocument) {
    if (!this.server) return;
    this.server.to(`auction:${auction._id.toString()}`).emit('auction-complete', {
      auctionId: auction._id,
