...
async function testAuctionFlow(): Promise<void> {
  ...
  // Verify balances
  const user1Balance = await api.functional.users.balance.getBalance(user1Conn);
  ...
  // Start auction
  const startedAuction = await api.functional.auctions.start(adminConn, auction.id);
  ...
  const bid1Result = await api.functional.auctions.bid.placeBid(user1Conn, auction.id, { amount: 100 });
  assertEqual(bid1Result.bid.amount, 100, 'Bid1 should be 100');
  console.log('✓ User1 placed bid: 100');

  // Verify balance frozen
  const user1After1 = await api.functional.users.balance.getBalance(user1Conn);
  ...
  const bid2Result = await api.functional.auctions.bid.placeBid(user2Conn, auction.id, { amount: 150 });
  ...
  const bid1Increase = await api.functional.auctions.bid.placeBid(user1Conn, auction.id, { amount: 200 });
  ...
  const user1After2 = await api.functional.users.balance.getBalance(user1Conn);
  ...
  try {
    await api.functional.auctions.bid.placeBid(user2Conn, auction.id, { amount: 50 });
  ...
  try {
    await api.functional.auctions.bid.placeBid(user2Conn, auction.id, { amount: 151 });
  ...
  // Min winning bid
  const minBid = await api.functional.auctions.min_winning_bid.getMinWinningBid(user1Conn, auction.id);
