...
async function testBalanceFreezing(): Promise<void> {
  ...
  // Deposit 1000
  await api.functional.users.deposit(userConn, { amount: 1000 });

  // Check initial balance
  let balance = await api.functional.users.balance.getBalance(userConn);
  ...
  balance = await api.functional.users.balance.getBalance(userConn);
...
async function testIncrementalFreeze(): Promise<void> {
  ...
  await api.functional.users.deposit(userConn, { amount: 2000 });
  ...
  await api.functional.auctions.bid.placeBid(userConn, auction.id, { amount: 800 });
  balance = await api.functional.users.balance.getBalance(userConn);
...
async function testOutbidRefund(): Promise<void> {
  ...
  await api.functional.users.deposit(user1Conn, { amount: 1000 });
  await api.functional.users.deposit(user2Conn, { amount: 1000 });
  ...
  // User1 bids 400
  await api.functional.auctions.bid.placeBid(user1Conn, auction.id, { amount: 400 });
  let user1Balance = await api.functional.users.balance.getBalance(user1Conn);
  ...
  user1Balance = await api.functional.users.balance.getBalance(user1Conn);
  ...
  const user2Balance = await api.functional.users.balance.getBalance(user2Conn);
...
async function testInsufficientFundsRejection(): Promise<void> {
  ...
  await api.functional.users.deposit(userConn, { amount: 500 });
  ...
  try {
    await api.functional.auctions.bid.placeBid(userConn, auction.id, { amount: 600 });
...
async function testMultiUserFinancialIntegrity(): Promise<void> {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    await api.functional.users.deposit(conn, { amount: deposits[i]! });
  ...
  for (let i = 0; i < 5; i++) {
    await api.functional.auctions.bid.placeBid(users[i]!.conn, auction.id, {
      amount: bids[i]!
    });
    await waitForLockRelease();
  }
  ...
  let totalInSystem = 0;
  for (let i = 0; i < 5; i++) {
    const balance = await api.functional.users.balance.getBalance(users[i]!.conn);
...
async function testBidValidation(): Promise<void> {
  ...
  try {
    await api.functional.auctions.bid.placeBid(userConn, auction.id, { amount: 50 });
  ...
  await api.functional.auctions.bid.placeBid(userConn, auction.id, { amount: 100 });
  ...
  try {
    await api.functional.auctions.bid.placeBid(userConn, auction.id, { amount: 105 });
  ...
  await api.functional.auctions.bid.placeBid(userConn, auction.id, { amount: 110 });
