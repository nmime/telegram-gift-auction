...
| **Высокая производительность** | Ультра-быстрые Redis Lua скрипты + WebSocket ставки достигают **~3,000 rps × количество CPU** с p99 латентностью менее 5мс |
...
### ⚡ WebSocket ставки (Максимальная производительность)
- **Прямые WebSocket ставки** полностью обходят HTTP overhead
- **~3,000 rps × количество CPU** с p99 латентностью менее 5мс
- **JWT аутентификация** через socket события
- **Real-time ответы** с мгновенным подтверждением

### 🔥 Кластерный режим (Горизонтальное масштабирование)
- **Multi-process scaling** через Node.js cluster модуль
- **CLUSTER_WORKERS=auto** автоматически использует все ядра CPU
- **Авто-рестарт** упавших воркеров
- **Redis adapter** синхронизирует Socket.IO между воркерами

### Real-time коммуникация
- WebSocket события: `new-bid`, `auction-update`, `anti-sniping`, `round-complete`
- **WebSocket ставки**: `auth` + `place-bid` события для ультра-низкой латентности
...
| **`CLUSTER_WORKERS`** | **Количество воркеров (`0`=один процесс, `auto`=все ядра)** | **0** |
