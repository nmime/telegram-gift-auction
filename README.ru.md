...
Где поведение Telegram не было явно задокументировано, приняты следующие допущения:

1. **Уникальные суммы ставок**: Два пользователя не могут иметь одинаковую активную ставку
2. **Минимальный шаг**: Увеличение ставки должно соответствовать минимальному шагу
3. **Лимит продлений**: Анти-снайпинг продления ограничены
4. **Немедленная заморозка**: Сумма ставки замораживается мгновенно
5. **Отмена ставок запрещена**: После размещения ставку нельзя отменить
6. **Участие ботов**: Симулированные боты для демонстрации (настраиваются для каждого аукциона)
